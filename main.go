...
func randomUserAgent() string {
	...
	randNum := rand.Int() % len(UserAgent)
...
func GoogleScrape(searchTerm, languageCode, countryCode string, proxyString interface{}, pages, count, backOff int) ([]SearchResult, error) {
	results := []SearchResult{}
	resultCounter := 0
	googlesPage, err := buildGoogleUrls(searchTerm, languageCode, countryCode, pages, count)
	if err != nil {
		return nil, err
	}
	for _, page := range googlesPage {
...
func main() {
	res, err := GoogleScrape("Hackathon", "en", "com", nil, 1, 30, 10) //to scraped(data ,language code ,country code, proxy string,no. of pages,no. of results,time)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
